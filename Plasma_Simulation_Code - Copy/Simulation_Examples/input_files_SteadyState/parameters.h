...
/*
 * This is a sample parameter file used to model a radio-frequency Argon
 * discharge with the rapid steady-state boundary implemented.
 *
 * This boundary inserts an electron-ion pair into the simulation domain
 * when an electron arrives at the surface. The particle number is constant
 * and ionization is switched off. The simulation runs for a 1000 RF cycles.
 *
 * The simulation takes about 2-3 hours to run on a computer with 4GB RAM,
...
/*
#define LOADING_DENSITY (double)1.0e17
#define ELECTRON_1D_DENSITY pow(LOADING_DENSITY,(1.0/3.0))
#define ION_1D_DENSITY pow(LOADING_DENSITY,(1.0/3.0))
#define ELECTRON_2D_DENSITY (double)1.0e7 // pow(LOADING_DENSITY,(2.0/3.0))
#define ION_2D_DENSITY (double)1.0e7  //pow(LOADING_DENSITY,(2.0/3.0))
#define E_MACROPARTICLE_SIZE ELECTRON_2D_DENSITY
#define ION_MACROPARTICLE_SIZE ION_2D_DENSITY
*/

#define E_MACROPARTICLE_SIZE 1.0e8
#define ION_MACROPARTICLE_SIZE 1.0e8

//Gas pressure in pascals.
#define GAS_PRESSURE ((double)0.2)*((double)133.32)
...
#define TOTAL_RUN_TIME ((int)1000*(int)PERIOD + 5)
//when to start collecting graph data points
#define START_GRAPH_TIME ((int)900*(int)PERIOD)
...
#define INIT_PARTICLE_NO (int)50000 // (ELECTRON_1D_DENSITY*LENGTH) 
...
//of the simulation
#define DXPE  (LENGTH/(INIT_PARTICLE_NO +(double)1.0))
#define DXPI  (LENGTH/(INIT_PARTICLE_NO +(double)1.0))
