...
/*
 * This is a sample parameter file used to model an ambipolar sheath
 *
 * The simulation takes about 50 minutes to run on a computer with 4GB RAM,
 * Intel core duo E8400 @3GHz processor, and a 64bit operating system.
 *
 * To turn off ionization select the constant option rather than energy-
 * dependent values, and then set the cross-section to zero.
 */
...
#define ELECTRON_2D_DENSITY (double)1.0e4 // pow(LOADING_DENSITY,(2.0/3.0))
#define ION_2D_DENSITY (double)1.0e4  //pow(LOADING_DENSITY,(2.0/3.0))
...
//in meters.
#define NO_OF_DEBYE_LENGTHS (int)200
...
#define DTI ((double)10.0*DT)
//Ratio of ion time-step over electron
#define DT_DTI (int)(DTI/DT)

#define ION_TRANSIT_TIME (0.5*LENGTH/(DTI*VTI))

//number of steps to take average of data over
#define NO_GRAPH_STEPS (int)(ION_TRANSIT_TIME)
//total run-time of simulation
#define TOTAL_RUN_TIME (int)(3.0*ION_TRANSIT_TIME)+ 5
//when to start collecting graph data points
#define START_GRAPH_TIME (int)(2.0*ION_TRANSIT_TIME)
...
#define INIT_PARTICLE_NO (ELECTRON_1D_DENSITY*LENGTH) 
